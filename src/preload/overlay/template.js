...
'    </div>',
'    <div class="cuckoo-actions">',
'      <button id="cuckoo-btn-settings" class="cuckoo-btn cuckoo-btn-secondary" title="打开设置">设置</button>',
'    </div>',
'    <div class="cuckoo-divider"></div>',
...
'  </div>',
'</div>',
'<div id="cuckoo-settings" class="cuckoo-window-manager cuckoo-hidden">',
'  <div class="cuckoo-wm-header">',
'    <span class="cuckoo-title">设置</span>',
'    <button id="cuckoo-settings-close" class="cuckoo-btn-icon" title="关闭">×</button>',
'  </div>',
'  <div class="cuckoo-wm-body">',
'    <div class="cuckoo-section">',
'      <label class="cuckoo-label"><input type="checkbox" id="cuckoo-retry-enabled" /> 失败自动重试</label>',
'      <div class="cuckoo-settings-row">',
'        <span class="cuckoo-settings-hint">普通失败间隔</span>',
'        <input id="cuckoo-retry-delay-min" type="number" min="0" step="1" value="4" class="cuckoo-settings-input" />',
'        <span class="cuckoo-settings-hint">至</span>',
'        <input id="cuckoo-retry-delay-max" type="number" min="0" step="1" value="10" class="cuckoo-settings-input" />',
'        <span class="cuckoo-settings-hint">秒</span>',
'      </div>',
'      <div class="cuckoo-settings-row">',
'        <span class="cuckoo-settings-hint">普通失败重试次数（负数=无限）</span>',
'        <input id="cuckoo-retry-count" type="number" step="1" value="10" class="cuckoo-settings-input" />',
'      </div>',
'      <div class="cuckoo-settings-row">',
'        <span class="cuckoo-settings-hint">操作频繁后重试间隔</span>',
'        <input id="cuckoo-retry-429-delay" type="number" min="0" step="1" value="60" class="cuckoo-settings-input" />',
'        <span class="cuckoo-settings-hint">秒</span>',
'      </div>',
'      <div class="cuckoo-settings-row">',
'        <span class="cuckoo-settings-hint">操作频繁后重试次数（负数=无限）</span>',
'        <input id="cuckoo-retry-429-count" type="number" step="1" value="20" class="cuckoo-settings-input" />',
'      </div>',
'      <div class="cuckoo-settings-row cuckoo-settings-row-col">',
'        <span class="cuckoo-settings-hint">重试提示词</span>',
'        <textarea id="cuckoo-retry-prompt" class="cuckoo-settings-textarea" rows="2"></textarea>',
'      </div>',
'      <div class="cuckoo-settings-row">',
'        <span class="cuckoo-settings-hint">AI 回复时间超时后重试</span>',
'        <input id="cuckoo-xhr-idle-timeout" type="number" min="0" step="1" value="300" class="cuckoo-settings-input" />',
'        <span class="cuckoo-settings-hint">秒</span>',
'      </div>',
'      <div class="cuckoo-settings-row">',
'        <span class="cuckoo-settings-hint">AI 回复时间超时后重试提示词</span>',
'        <input id="cuckoo-watchdog-prompt" type="text" value="请继续" class="cuckoo-settings-input" style="width:160px;" />',
'      </div>',
'      <div class="cuckoo-settings-row">',
'        <span class="cuckoo-settings-hint">AI 回复时间超时重试次数（负数=无限）</span>',
'        <input id="cuckoo-watchdog-count" type="number" step="1" value="3" class="cuckoo-settings-input" />',
'      </div>',
'    </div>',
'    <div class="cuckoo-divider"></div>',
'    <div class="cuckoo-section">',
...
'      <div class="cuckoo-settings-row">',
'        <input id="cuckoo-delay-min" type="number" min="0" step="0.1" value="2" class="cuckoo-settings-input" />',
'        <span class="cuckoo-settings-hint">至</span>',
'        <input id="cuckoo-delay-max" type="number" min="0" step="0.1" value="4" class="cuckoo-settings-input" />',
'        <span class="cuckoo-settings-hint">秒</span>',
'      </div>',
'    </div>',
'    <div class="cuckoo-actions">',
'      <button id="cuckoo-settings-save" class="cuckoo-btn cuckoo-btn-primary">保存设置</button>',
'      <button id="cuckoo-settings-reset" class="cuckoo-btn cuckoo-btn-secondary">恢复默认</button>',
'    </div>',
'  </div>',
'</div>',
].join('\n');
...
'.cuckoo-retry-countdown {',
'  position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%);',
'  z-index: 2147483647; pointer-events: auto;',
'}',
'.cuckoo-retry-countdown.cuckoo-hidden { display: none !important; }',
'.cuckoo-retry-countdown-inner {',
'  display: flex; align-items: center; gap: 14px;',
'  background: rgba(17, 19, 34, 0.98); border: 1px solid var(--ck-border);',
'  border-radius: 12px; padding: 10px 16px;',
'  box-shadow: 0 8px 28px rgba(0, 0, 0, 0.45);',
'  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif;',
'  font-size: 13px; color: #dde1ff;',
'}',
'#cuckoo-retry-countdown-text { white-space: nowrap; }',
'#cuckoo-retry-cancel { color: #8a90b8; cursor: pointer; }',
'#cuckoo-retry-cancel:hover { color: #ff6b7a; }',
'#cuckoo-settings { width: 420px; max-width: 92vw; }',
'.cuckoo-settings-row { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }',
'.cuckoo-settings-row-col { flex-direction: column; align-items: stretch; gap: 4px; }',
'.cuckoo-settings-hint { font-size: 12px; color: var(--ck-text-dim); white-space: nowrap; }',
'.cuckoo-settings-input { width: 90px; background: var(--ck-code-bg); border: 1px solid var(--ck-border); border-radius: 6px; padding: 4px 8px; color: var(--ck-text); font-size: 12px; }',
